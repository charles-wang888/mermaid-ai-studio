...
与传统的图形生成工具直接基于用户文本进行绘制不同，本系统引入了智能需求澄清机制。在生成图表前，系统首先基于用户意图对需求进行智能分析和自动拆解，将复杂的架构需求转化为结构化的任务列表（TODO），帮助用户系统化地梳理设计要点。随后，通过 AI 驱动的交互式问答，自动识别需求中的模糊点、缺失信息和潜在矛盾，针对每个任务项进行深度澄清，确保在绘制前充分理解用户意图。这种"先拆解、再澄清、后绘制"的设计理念，显著提升了生成图表的准确性和完整性，避免了因需求理解偏差导致的重复修改。
...
9. **智能自动修复**：针对每种图表类型的常见语法错误，提供专门的修复器：
   - **时序图**：自动修复参与者名称分割问题（如 `participant LogSt as h` → `participant LogStash`）
   - **类图**：自动修复方法定义冒号错误、缺失 `class` 关键字、关系符号错误等
   - **状态图**：自动修复嵌套状态中的 `*` 为 `[*]`、状态名称引号等
   - **甘特图**：自动移除 `taskId`、修正任务格式、修复日期计算
   - **饼图**：自动添加缺失的 `title` 关键字
   - **象限图**：自动归一化坐标值（百分比和市场份额自动转换为 0-1 范围）
   - **用户旅程图**：自动修复格式错误（将情感名称格式转换为分数格式）
10. **文本清理增强**：自动清理 TODO 列表中的 HTML 标签（`<div>`, `</div>`）和 Markdown 符号（`**`, `*` 等）
11. **图表缩放控制**：支持放大、缩小、重置视图
12. **多格式导出**：支持 PNG 图片和 Mermaid 源码文件导出
...
│   │   ├── pie_chart_fixer.py      # 饼图语法修复器
│   │   ├── quadrant_chart_fixer.py # 象限图语法修复器
│   │   └── journey_fixer.py       # 用户旅程图语法修复器
│   ├── parsers/                    # 解析器模块
...
每种图表类型都有对应的语法修复器，负责自动检测和修复特定类型的语法错误。修复器采用策略模式设计，支持独立的修复逻辑和统一的接口：

- **`base_fixer.py`**：修复器基类，定义所有修复器的统一接口
- **`fixer_factory.py`**：修复器工厂，根据图表类型自动创建对应的修复器实例

#### 各类型修复器功能详情

**流程图修复器 (`flowchart_fixer.py`)**：
- 优化流程图布局（自动将步骤较多的 LR 布局转换为 TD 布局）
- 修复子图连接问题
- 清理节点标签中的特殊字符

**时序图修复器 (`sequence_fixer.py`)**：
- 修复 `participant` 和 `actor` 定义中的多个 `as` 关键字错误
- 自动合并被分割的参与者名称（如 `participant LogSt as h as 数据采集组件` → `participant LogStash as "数据采集组件"`）
- 为包含中文或空格的表现名称添加引号

**类图修复器 (`class_diagram_fixer.py` / `class_diagram_fixer_advanced.py`)**：
- 移除重复的 `classDiagram` 声明
- 修复方法定义中的冒号错误（`方法名(): 返回类型` → `方法名() 返回类型`）
- 修复缺少 `class` 关键字的类定义
- 修复关系符号错误（如 `--|>` → `-->`）
- 推断和添加缺失的关系定义
- 重新组织代码结构（类定义在前，关系定义在后）
- 去重类定义和关系

**状态图修复器 (`state_diagram_fixer.py`)**：
- 修复状态定义中的空格和中文引号问题
- 修复嵌套状态中的 `*` 为 `[*]`
- 修复转换条件格式（从 `{条件}` 改为 `: "条件"`）
- 为包含中文或空格的状态名称和转换条件添加引号
- 去重状态转换关系

**甘特图修复器 (`gantt_fixer.py`)**：
- 移除无效的 `taskId` 参数
- 修正任务格式（统一为 `任务名称 :状态, 开始日期, 持续天数`）
- 修复日期和持续时间的计算
- 为包含空格的任务名称添加引号（注意：中文不需要引号，避免解析错误）
- 修复里程碑格式

**饼图修复器 (`pie_chart_fixer.py`)**：
- 自动添加缺失的 `title` 关键字（将 `pie` 修复为 `pie title 数据分布`）
- 确保数据项格式正确（标签用引号包裹）
- 验证数值格式

**象限图修复器 (`quadrant_chart_fixer.py`)**：
- 为 x-axis 和 y-axis 的中文标签添加引号
- 为 quadrant-1 到 quadrant-4 的中文标签添加引号
- **自动坐标归一化**：
  - X 轴坐标：市场增长率百分比自动转换为 0-1 范围（如 40% → 0.8）
  - Y 轴坐标：相对市场份额自动转换为 0-1 范围（如 1.8 → 0.9）
- 确保所有坐标在 0-1 范围内

**用户旅程图修复器 (`journey_fixer.py`)**：
- 修复错误的格式（将 `任务: 情感名称: 参与者, 分数` 修复为 `任务: 分数: 参与者`）
- 移除情感名称，只保留分数和参与者
- 确保分数在 1-5 范围内
- 规范化任务步骤格式
...
- **`text_cleaner.py`**：清理和标准化文本内容，包括：
  - 移除所有 HTML 标签（`<div>`, `</div>`, `<span>` 等）
  - 清理 Markdown 格式化符号（`**`, `*`, `__`, `_` 等）
  - 清理 HTML 实体（`&nbsp;`, `&amp;` 等）
  - 为 TODO 列表和问题文本提供统一的清理功能
