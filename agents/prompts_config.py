...
3. 定义状态：
   - [*] 表示起始状态
   - [*] 表示结束状态（用于终止状态转换）
   - **重要**：简单状态可以直接使用状态名称，不需要定义 state 块
   - **重要**：Mermaid状态图**不支持 description: 语法**，不要在状态中使用 description:
   - 只有包含嵌套状态的复合状态才需要 state 状态名 {{ }} 语法
   - **错误示例**：
     state "状态名" {{
       description: 状态描述
     }}
   - **正确示例**：
     - 简单状态：直接使用状态名（如："待支付"），通过转换语句自动定义
     - 嵌套状态：state 复合状态 {{
         state 子状态1
         state 子状态2
       }}
4. 定义状态转换（**这是最重要的部分**）：
   ...
   - 转换条件可以包含中文和空格，会自动处理
5. **状态定义方式**：
   - **推荐**：不单独定义状态，直接在转换语句中使用状态名，Mermaid会自动识别
   - **示例**：
     [*] --> "待支付"
     "待支付" --> "已支付" : 用户支付
     "已支付" --> [*]
   - **如果需要嵌套状态**，才使用 state 块定义
6. 确保状态转换逻辑完整清晰，包含所有状态和转换关系
7. 终止状态（最终状态）使用 状态名 --> [*] 来表示
8. 只输出Mermaid代码，不要markdown代码块标记

请生成详细的状态图代码（**不要使用 description: 语法，直接使用状态转换来定义状态**）：
"""
...
2. **第一行必须是**：pie title 图表标题（必须包含 title 关键字，不能只有 pie）
   - **错误示例**：pie
   - **正确示例**：pie title 公司各部门预算分配
3. 定义数据项（每行一个）：
   - "标签" : 数值
   - 例如："研发部门" : 350
   - 数值可以是整数或小数
4. **重要**：数据项格式必须正确，标签必须用双引号包裹
5. 确保所有数据项的总和合理
6. 只输出Mermaid代码，不要markdown代码块标记
...
6. 定义数据点：
   - 格式：名称: [x坐标, y坐标]
   - **坐标转换规则**：
     * X轴坐标 = 市场增长率百分比 / 50（例如：40% -> 40/50 = 0.8）
     * Y轴坐标 = 相对市场份额 / 2.0（例如：1.8 -> 1.8/2.0 = 0.9）
   - **重要**：坐标值必须是0到1之间的小数
   - **重要**：点名称不能使用中文"点"字前缀，直接使用英文或中文名称
   - 正确示例：A: [0.8, 0.9]（40%增长率，1.8市场份额）
   - 错误示例：点A: [0.5, 0.8]（不要使用"点"前缀）或 A: [40, 1.8]（未归一化）
7. **必须包含所有数据点**，不要遗漏任何产品
8. 确保数据点分布在合适的象限
8. 只输出Mermaid代码，不要markdown代码块标记
...
4. 定义阶段和步骤：
   - section 阶段名称
   - **重要**：步骤格式必须是 "步骤名称: 分数: 参与者"（注意：是分数在前，不是情感状态）
   - 格式：步骤名称: 分数: 参与者
   - 分数：1-5的整数（表示该步骤的重要程度或满意度）
   - 参与者：用户或系统
   - **错误格式**：步骤名称: 情感状态: 参与者, 分数
   - **正确格式**：步骤名称: 分数: 参与者
5. 例如：
   - section 浏览阶段
   - 浏览商品: 4: 用户
